...
define
   local
      ...
      Tests = [ %% each element is [listComprehension]#[expectedList]
                %% add tests form here...
                [c:collect:C suchthat A in 1..2 do {C A}]
                #'#'(c:[1 2])

                [c:collect:C suchthat A in 1..2 if A == 1 do {C A}{C A+1}]
                #'#'(c:[1 2])

                [c:collect:C suchthat _ in 1..1 A from fun{$}1 end do {C A}{C A+1}]
                #'#'(c:[1 2])

                [c:collect:C suchthat _:A in r(r(r(1) r(2))) do {C A}]
                #'#'(c:[1 2])

                [c:collect:C suchthat A in 1..2 suchthat B in 3..4 do {C A+B}{C A*B}]
                #'#'(c:[4 3 5 4 5 6 6 8])

                [1:collect:C1 2:collect:C2 suchthat A in 1..2 do {C1 A}{C1 A*A}{C2 A+1}]
                #([1 1 2 4]#[2 3])

                [1:collect:C1 2:A+1 suchthat A in 1..2 do {C1 A}{C1 A*A}]
                #([1 1 2 4]#[2 3])

                [c:collect:C suchthat A in 1..3 if local skip in {C A} 0 == 1 end]
                #'#'(c:[1 2 3])

                [c:collect:C suchthat A in 1..3 if {Ext C false} do {C A}]
                #'#'(c:[1 1 1])

                [c:collect:C suchthat A in 1..3 if {Ext C true} do {C A}]
                #'#'(c:[1 1 1 2 1 3])
                %% ...to here
              ]
